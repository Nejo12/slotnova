...
## `GET /calendar?resourceId=&from=&to=`
...
- Response `200`: a merged view model — open intervals (from Scheduling's
  ...
  list) for the requested resource/date range, bounded by the same
  expansion horizon as `scheduling.contract.md`'s resolve endpoint.
- Failures:
